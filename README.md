...
```
workstation-mission/
├── README.md              # 실습 전체 기록 및 학습 정리
├── Dockerfile             # 커스텀 이미지 빌드 정의
├── site/                  # Dockerfile COPY 대상 (빌드용 정적 파일)
│   └── index.html
├── bind-site/             # 바인드 마운트 실습용 (런타임 실시간 반영)
│   └── index.html
├── docs/                  # 증거 자료 (스크린샷)
│   └── images/
│       ├── browser-8080.png
...
├── perm_file.txt          # 권한 실습용 파일
└── perm_dir/              # 권한 실습용 디렉터리
```
...
| 디렉터리 | 역할 | 분리 이유 |
|----------|------|----------|
| `site/` | Dockerfile에서 `COPY`로 이미지에 포함되는 파일 | 빌드 타임에 이미지 안에 고정되는 파일은 별도 관리 |
| `bind-site/` | 바인드 마운트로 컨테이너에 실시간 연결되는 파일 | 런타임에 호스트에서 수정하면 즉시 반영되므로 `site/`와 분리 |
| `docs/images/` | 실습 증거 스크린샷 | 코드·설정과 무관한 문서 자료는 `docs/` 하위로 격리 |
| 루트 파일들 | `Dockerfile`, `README.md` 등 프로젝트 핵심 파일 | 프로젝트 진입 시 바로 확인할 수 있도록 최상위에 배치 |

> `site/`(빌드용)와 `bind-site/`(런타임 마운트용)를 분리한 것이 핵심이다. 같은 폴더를 쓰면 빌드 이미지에 포함된 파일과 바인드 마운트 파일이 혼재되어 어떤 변경이 어디에 반영되는지 추적이 어려워진다.
...
### 트러블슈팅 2 : 바인드 마운트 경로 오류

- **문제** : 바인드 마운트 실행 시 경로 관련 오류
- **원인 가설** : `bind-site` 폴더가 존재하지 않거나 경로가 잘못됨
- **확인** : `pwd`, `ls -la`, `ls -la bind-site`로 경로 확인
- **해결** : `bind-site` 폴더 생성 후 다시 시도

### 트러블슈팅 3 : 포트 매핑 실패 (호스트 포트 충돌)

만약 컨테이너 실행 시 `Bind for 0.0.0.0:8080 failed: port is already allocated` 에러가 발생한다면, 아래 순서로 진단한다.

```bash
# 1단계: 해당 포트를 점유 중인 프로세스 확인
lsof -i :8080

# 2단계: 해당 포트를 사용 중인 Docker 컨테이너 확인
docker ps --filter "publish=8080"

# 3단계-A: 기존 컨테이너가 원인이면 중지/삭제
docker stop <컨테이너명>
docker rm <컨테이너명>

# 3단계-B: Docker 외 프로세스가 원인이면 해당 프로세스 종료하거나 다른 포트 사용
docker run -d --name workstation-web -p 8888:80 workstation-web:1.0
```

**진단 흐름 요약:**
1. `lsof -i :<포트>`로 점유 프로세스 PID 확인
2. Docker 컨테이너인지 시스템 프로세스인지 구분
3. 원인에 따라 컨테이너 삭제 또는 호스트 포트 변경으로 해결
...
| 구분 | 절대 경로 | 상대 경로 |
|------|----------|----------|
| 형식 | `/`(루트)부터 시작 | 현재 위치(`.`)로부터 시작 |
| 예시 | `/Users/seongwoo/Documents/sw-codyssey` | `./bind-site/index.html` |
| 특징 | 어디서 실행해도 같은 위치를 가리킴 | 현재 디렉터리에 따라 달라짐 |

**상황별 선택 기준:**

| 상황 | 선택 | 이유 |
|------|------|------|
| Docker 바인드 마운트 (`-v`) | **절대 경로** 또는 `$(pwd)` | Docker는 상대 경로를 볼륨명으로 해석하므로 반드시 절대 경로 필요 |
| 스크립트 내부 경로 참조 | **절대 경로** | 스크립트가 어느 위치에서 실행될지 모르므로 정확한 경로 보장 |
| 프로젝트 내부 파일 참조 | **상대 경로** | 프로젝트를 다른 위치로 이동해도 경로가 유효 |
| Dockerfile `COPY` 명령 | **상대 경로** | 빌드 컨텍스트(`.`) 기준으로 동작하며 이식성 확보 |
| Git 추적 대상 | **상대 경로** | 협업 시 각자의 절대 경로가 다르므로 상대 경로가 필수 |

> **실무 원칙**: 프로젝트 내부 → 상대 경로, 시스템/Docker 설정 → 절대 경로. Docker `-v` 옵션에서 상대 경로를 쓰면 볼륨 이름으로 인식되므로 `$(pwd)/상대경로`로 변환해야 한다.
...
### 8) 포트 매핑과 컨테이너 네트워크 격리

```
호스트 네트워크                      컨테이너 네트워크 (격리됨)
┌──────────────┐                 ┌──────────────────┐
...
Docker 컨테이너는 기본적으로 **독립된 네트워크 네임스페이스**에서 실행된다. 컨테이너는 호스트와 다른 IP 대역(예: `172.17.0.0/16`)을 갖고, 호스트에서 컨테이너 내부 IP로 직접 접근할 수 없다. 이 격리 덕분에 여러 컨테이너가 동일한 내부 포트(예: 모두 `:80`)를 사용해도 충돌하지 않는다.

외부에서 컨테이너 서비스에 접근하려면 `-p 호스트포트:컨테이너포트` 옵션으로 **포트 매핑**을 명시해야 한다.

```bash
# 여러 컨테이너가 내부적으로 동일한 80번 포트를 사용하지만, 호스트 포트를 다르게 매핑
docker run -d -p 8080:80 --name web1 nginx:alpine   # localhost:8080 → web1:80
docker run -d -p 8081:80 --name web2 nginx:alpine   # localhost:8081 → web2:80
```

> **핵심**: `-p` 없이 `EXPOSE`만 선언하면 컨테이너 간 통신에만 사용되고, 호스트에서는 접근할 수 없다.
...
이 실습에서 사용한 포트/볼륨/마운트 설정을 누구나 동일하게 재현할 수 있도록 다음과 같이 정리하였다.

**포트 매핑 설정:**

| 서비스 | 호스트 포트 | 컨테이너 포트 | 용도 |
|--------|:---------:|:----------:|------|
| workstation-web | 8080 | 80 | 커스텀 빌드 이미지 확인 |
| bind-web | 8081 | 80 | 바인드 마운트 실시간 반영 확인 |

**볼륨/마운트 설정:**

| 컨테이너 | 마운트 방식 | 소스 | 대상 | 목적 |
|----------|-----------|------|------|------|
| bind-web | 바인드 마운트 | `$(pwd)/bind-site` | `/usr/share/nginx/html` | 호스트 파일 수정 즉시 반영 |
| vol-test | 볼륨 | `mydata` | `/data` | 컨테이너 삭제 후 데이터 영속성 검증 |
...
# 2. 커스텀 이미지 컨테이너 실행 (포트 8080)
docker run -d --name workstation-web -p 8080:80 workstation-web:1.0

# 3. 바인드 마운트 컨테이너 실행 (포트 8081)
docker run -d --name bind-web -p 8081:80 \
  -v "$(pwd)/bind-site:/usr/share/nginx/html" nginx:alpine
...
> 위 명령을 프로젝트 루트(`workstation-mission/`)에서 순서대로 실행하면 동일한 환경을 재현할 수 있다. 모든 경로는 `$(pwd)` 기반 상대 참조를 사용하여 프로젝트 위치에 독립적이다.
